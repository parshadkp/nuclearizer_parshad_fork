...
bool MDetectorEffectsEngineSMEX::Initialize()
{
  ...
  m_ShieldDelayAfter = 1.2e-6; // SCB delay before and after an event
  m_ShieldVetoWindowSize = m_ShieldPulseDuration + m_ShieldDelayBefore + m_ShieldDelayAfter; // this will change depending on number of ASIC channels read out.
...
bool MDetectorEffectsEngineSMEX::GetNextEvent(MReadOutAssembly* Event)
{
  ...
  while ((SimEvent = m_Reader->GetNextEvent(false)) != nullptr) {
    ...
    if (increaseShieldDeadTime == true){ m_ShieldDeadTime += m_ShieldVetoWindowSize; }
    ...
    //(3) shield active during the entire veto window
    //this if statement could perhaps be condensed but I'm less confused this way
    if ((m_ShieldTime + m_ShieldDelay > evt_time && m_ShieldTime + m_ShieldDelay < evt_time + m_ShieldVetoWindowSize) || 
      (m_ShieldTime + m_ShieldDelay + m_ShieldPulseDuration + m_ShieldDeadTime > evt_time && m_ShieldTime + m_ShieldDelay + m_ShieldPulseDuration + m_ShieldDeadTime> evt_time + m_ShieldVetoWindowSize) || 
      (m_ShieldTime + m_ShieldDelay < evt_time && m_ShieldTime + m_ShieldDelay + m_ShieldPulseDuration + m_ShieldDeadTime > evt_time + m_ShieldVetoWindowSize)){
      // 		  delete SimEvent;
      //      continue;
      //don't delete the event yet: need to apply dead time to the card cage first!
      m_ShieldVeto = true;
        ...
        for (int det=0; det<nDets; det++){
          if (detectorsHitForShieldVeto[det] == 1){
            ...
            if (evt_time > m_LastHitTimeByDet[det] + m_ASICDeadTime[det]){
              ...
              m_TotalDeadTime[det] += m_ASICDeadTime[det] + m_ShieldVetoWindowSize;
            }
          }
        }
        delete SimEvent;
  ...
  */
      ...
      for (int d=0; d<nDets; d++){
        if (updateLastHitTime[d] == 1){
          ...
          m_TotalDeadTime[d] += m_ASICDeadTime[d] + m_ShieldVetoWindowSize;
        }
      }
